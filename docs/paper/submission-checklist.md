...
| 论文 | 投稿候选稿完成 | `docs/paper/04_submission_manuscript_candidate.md`；本地 DOCX 可由 `src/33_export_submission_docx.py` 生成并默认附带表格附录 | 排版/压缩字数/人工改写 |
...
- [x] 生成本地 Word 初稿：`python3 src/33_export_submission_docx.py`（默认追加 Table 1–13 表格附录，供 Word/WPS 内移动到正文）。
...
- [x] 本地提交包自动包含 `paper/04_submission_manuscript_candidate.docx`，并在文末附带当前表格附录。
