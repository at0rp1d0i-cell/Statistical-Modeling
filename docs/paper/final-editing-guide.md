...
5. 从 `outputs/figures/` 插入图件：优先使用 PDF；若 Word/WPS 对 PDF 支持不好，使用 PNG；JPG 仅作为兼容备选。建议先把 Figure 10 研究框架图移到引言末尾或方法章节开头。
...
| 图号 | 建议位置 | 文件 | 编辑提醒 |
| --- | --- | --- | --- |
| Figure 10 | 引言末尾或研究方法开头 | `figure_10_research_framework` | 研究框架与技术路线图；只概括既有证据链，不作为新增实证结果。 |
