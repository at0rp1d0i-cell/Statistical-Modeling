...
- 论文可评审初稿 `docs/paper/03_manuscript_v0_2.md` 已建立
...
已新增 `docs/paper/03_manuscript_v0_2.md` 作为当前最完整正文底稿。相比 v0.1，v0.2 已经：

- 将当前主结果、稳健性、人口敏感性、异质性和政策文本 readiness 边界整合为完整论文叙述；
- 增加更完整的理论机制、方法说明、讨论、政策含义和局限性；
- 在文末补充表图位置建议和复现命令；
- 明确政策文本 `Table 13` 当前为 `not_ready`，不把 LLM 模块写成最终机制证据。

使用边界：v0.2 可用于内部评审和答辩结构演练；正式提交前仍需补齐参考文献格式、表图排版、AI 使用说明、数据代码包说明，并在最终规格锁定后重跑关键表图。
