...
def write_package_manifest(
    ...
    lines = [
        ...
        "## 内容结构",
        "",
        "- `paper/`：论文 Markdown 初稿、带核心表格摘录和图件清单的 Word 初稿、最终 Word 编辑指南、反向评估素材清单、参考文献、提交清单和表图清单。",
        ...
        "- 主线题目：数字普惠金融的碳减排效应。",
        "- 主结果变量：碳排放强度。",
        "- 当前样本：2019—2023 年、294 个城市、1456 个城市—年份观测。",
        "- 人口变量不进入主回归，只作为敏感性检验。",
        ...
        f"- 已复制资产数：{len(copied)}",
        f"- 缺失资产数：{len(missing)}",
        f"- Word 初稿：{'已生成（仅附核心表格摘录和图件清单；完整表格在 outputs/tables/）' if docx_generated else '未生成'}",
